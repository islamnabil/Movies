...
open class BaseAPI<T: TargetType>: NSObject {
    ...
    public func sendRequest(target: T) -> AnyPublisher<Any?, ErrorResponse> {
    ...
    public func fetchData<M: Decodable>(target: T, shouldUseCache: Bool = false) -> AnyPublisher<M, ErrorResponse> {
        var error: NSError = genericError
        let method = Alamofire.HTTPMethod(rawValue: target.method.rawValue)
        let headers = Alamofire.HTTPHeaders(target.headers ?? [:])
        let params = buildParams(task: target.task)
    ...
    public func upload<M: Decodable>(target: T, shouldUseCache: Bool = false) -> AnyPublisher<M, ErrorResponse> {
        var error: NSError = genericError
        let method = Alamofire.HTTPMethod(rawValue: target.method.rawValue)
        let headers = Alamofire.HTTPHeaders(target.headers ?? [:])
